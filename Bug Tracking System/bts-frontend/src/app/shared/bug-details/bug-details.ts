...
import { Bug, BugPriority, BugResponse, BugStatus } from '../../models/bug.model';
...
export class BugDetails implements OnInit {
  ...
  this.bugService.getBugById(id).subscribe({
    next: (response: any) => {
      const cleanResponse = {
        ...response,
        parentBugIds: response?.parentBugIds?.$values || []
      };

      this.bug = cleanResponse;
